...
def simulator_lip():
    ...
    eyeliner_makeup = Eyeliner()
    
    img = eyeliner_makeup.apply_eyeliner(
        cropped_img,landmarks_x, landmarks_y, r_value, g_value, b_value, 1.2)
    ...
    predict_result_intense = save_iamge(user_image_copy,r_value,g_value,b_value,"eyeshadow",1.2)

    img = eyeliner_makeup.apply_eyeliner(
        cropped_img,landmarks_x, landmarks_y, r_value, g_value, b_value, 0.9)
    img = imutils.resize(img, width=new_x2-new_x1)
    cheight, cwidth = img.shape[:2]
    user_image_copy = user_image.copy()
    user_image_copy[ new_y1:new_y1+cheight, new_x1:new_x1+cwidth] = img
    user_image_copy = cv2.cvtColor(user_image_copy, cv2.COLOR_BGR2RGB)
    predict_result_medium = save_iamge(user_image_copy,r_value,g_value,b_value,"eyeshadow",0.9)
    ...
    user_image_copy[ new_y1:new_y1+cheight, new_x1:new_x1+cwidth] = img
    user_image_copy = cv2.cvtColor(user_image_copy, cv2.COLOR_BGR2RGB)
    predict_result_fade = save_iamge(user_image_copy,r_value,g_value,b_value,"eyeshadw",0.7)
